    ...
    <div class="container">
      <div class="row">
        <div class="col-12">
          <h1>Set Goals</h1>
          <p>
            <input id="o-editGroups__toggleUserCurrentGoals" v-model="toggleShowOnlyUserGoals" type="checkbox">
            <label for="o-editGroups__toggleUserCurrentGoals">Show only current goals</label>
          </p>
        </div>
        <ul id="" class="o-editGroups__cardContainer col-12">
          <li v-for="(group, group_key) in skillGroups">
            <h2>{{ group.name }}</h2>
            <div class="container">
              <div class="row">
                <template v-for="skill in group.skills">
                  <div v-if="toggleShowOnlyUserGoals === false || (toggleShowOnlyUserGoals === true && getCurrentGoalLevel(skill.s_id) !== '—')" class=" col-6 col-md-4 mb-4 ">
                    <div class="o-editGroups__skillCard p-4 " :class="{'-isRequired': skill.required}">
                      <p>
                        {{ skill.name }}
                        <br>
                        Current Level &amp; Goal: {{ getCurrentSkillLevel(skill.s_id) }} /
                        {{ getCurrentGoalLevel(skill.s_id) }} </p>
                      <div class="d-flex">
                        <button v-for="level in skillLevels" v-bind:class="{'-targetLevel': currentGoalLevels[skill.s_id][level.level], '-active': currentSkillLevel[skill.s_id][level.level]}" class="flex-grow-1" @click="setSkill(level, group_key, skill)">
                          {{ level.level }}
                        </button>
                      </div>
                    </div>
