        ...
        <div class="container">
            <div class="row">
                <div class="col-12">
                    <h1>Set Goals</h1>
                    <p>
                        <input id="o-editGroups__toggleUserCurrentGoals" type="checkbox" v-model="toggleShowOnlyUserGoals">
                        <label for="o-editGroups__toggleUserCurrentGoals">Show only current goals</label>
                    </p>
                </div>
                <ul class="o-editGroups__cardContainer col-12" id="">
                    <li v-for="(group, group_key) in skillGroups">
                        <h2>{{ group.name }}</h2>
                        <div class="container">
                            <div class="row">
                                <template v-for="skill in group.skills">
                                    <div class=" col-6 col-md-4 mb-4 " v-if="toggleShowOnlyUserGoals === false || (toggleShowOnlyUserGoals === true && getCurrentGoalLevel(skill.s_id) !== '—')">
                                        <div :class="{'-isRequired': skill.required}" class="o-editGroups__skillCard p-4 ">
                                            <p>
                                                {{ skill.name }}
                                                <br>
                                            ...
                                            <div class="d-flex">
                                                <button @click="setSkill(level, group_key, skill)" class="flex-grow-1" v-bind:class="{'-targetLevel': currentGoalLevels[skill.s_id][level.level], '-active': currentSkillLevel[skill.s_id][level.level]}" v-for="level in skillLevels">
                                                    {{ level.level }}
                                                </button>
                                            </div>
                                        </div>
